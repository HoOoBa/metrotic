...
class _HomeState extends State<Home> with SingleTickerProviderStateMixin {
  ...
  void checkTicketsExpiration() {
    FirebaseFirestore.instance.collection('Users'.tr()).get().then((value) {
      for (int i = 0; i < value.docs.length; i++) {
        String userUid = value.docs[i]['uid'];
        print(userUid);
        FirebaseFirestore.instance.collection('Tickets'.tr()).get().then((value) {
          for (int j = 0; j < value.docs.length; j++) {
            String tickedUid = value.docs[j]['uid'];
            print(tickedUid);
            if (userUid == tickedUid &&
                DateFormat.yMMMd('en_US')
                    .parse((value.docs[j].data()['date'.tr()]))
                    .isBefore(DateTime.now())) {
              print('yes'.tr());
              NotificationService.showNotification(
                  title: 'Ticket Expired'.tr(),
                  body: 'Please renew your ticket'.tr(),
                  payload: 'expired'.tr(),
                  context: context);
            }
          }
      ...
      markers.add(Marker(
        //add first marker
          ...
          infoWindow: InfoWindow(
            ...
            snippet: 'Shark El Seka El Hadid, Shubra El Kheima'.tr(),
          ),
      ...
      markers.add(Marker(
        //add third marker
          ...
          infoWindow: InfoWindow(
            //popup info
              title: 'El Mazalat'.tr(),
              snippet: 'El Mazalat Bridge, Cairo'.tr()),
          icon: await BitmapDescriptor.fromBytes(markerIcon)));
      markers.add(Marker(
        //add third marker
          markerId: MarkerId("5"),
      ...
      markers.add(Marker(
        //add third marker
          ...
          infoWindow: InfoWindow(
            //popup info
            title: 'St. Teresa '.tr(),
            snippet: 'Sherif, As Sahel, Cairo Governorate, Egypt'.tr(),
          ),
          icon: await BitmapDescriptor.fromBytes(markerIcon)));
      markers.add(Marker(
        //add third marker
          ...
          infoWindow: InfoWindow(
            //popup info
            title: 'Rod El-Farag'.tr(),
            snippet: 'Al Barad, As Sahel, Cairo Governorate, Egypt'.tr(),
          ),
          icon: await BitmapDescriptor.fromBytes(markerIcon)));
      markers.add(Marker(
        //add third marker
      ...
      markers.add(Marker(
        //add third marker
          ...
          infoWindow: InfoWindow(
            ...
            snippet: 'Cairo, Cairo Governorate, Egypt'.tr(),
          ),
          icon: await BitmapDescriptor.fromBytes(markerIcon)));
      markers.add(Marker(
        //add third marker
          ...
          infoWindow: InfoWindow(
            ...
            snippet: 'Al Gomhoreya St., Cairo, Cairo Governorate, Egypt'.tr(),
          ),
          icon: await BitmapDescriptor.fromBytes(markerIcon)));
      markers.add(Marker(
        //add third marker
